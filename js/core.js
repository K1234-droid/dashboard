...
export function translateUI(lang) {
    ...
    document.querySelectorAll('[data-i18n-key]').forEach(el => {
        ...
        const ariaLabelKey = el.getAttribute('data-i18n-key-aria-label');
        if (ariaLabelKey) {
            const ariaTranslation = i18nData[ariaLabelKey]?.[lang] || i18nData[ariaLabelKey]?.['id'];
            if (ariaTranslation) {
...
async function checkRealInternetConnection(retries = 2, delay = 500) {
    for (let i = 0; i < retries; i++) {
        ...
        try {
            ...
            log('info', 'log.info.connectionCheckFailed', { attempt: i + 1 }, error.name);
            if (i < retries - 1) {
...
export async function checkForUpdates(isManual = false) {
    ...
    try {
        ...
        log('info', 'log.error.updateCheckFailed', {}, error);
        if (isManual) {
