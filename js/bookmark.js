...
import { showToast, isValidURL, log } from './utils.js';
import { saveSetting, saveFaviconToCache, getFaviconFromCache, deleteFaviconFromCache } from './storage.js';
...
function getFavicon(url) {
    if (!url || !isValidURL(url)) {
        return '../assets/icons/404-icon16.webp';
    }
...
async function setFaviconWithFallback(imgElement, url) {
    const fallbackSrc = '../assets/icons/404-icon16.webp';
    ...
    try {
        const cachedBlob = await getFaviconFromCache(domain);
        if (cachedBlob) {
        ...
        if (newBlob && newBlob.type.startsWith('image/')) {
            imgElement.src = URL.createObjectURL(newBlob);
            await saveFaviconToCache(domain, newBlob);
        }
    } catch (error) {
        log('info', 'log.error.fetchFavicon', { domain: domain }, error);
    }
}

export function renderMainPageBookmarks() {
    if (!elements.mainPageBookmarkContainer || !elements.mainPageBookmarkControls) return;

    elements.mainPageBookmarkContainer.innerHTML = '';
    elements.mainPageBookmarkControls.innerHTML = '';
    ...
    if (contextMenuMain) {
        contextMenuMain.addEventListener('click', (e) => {
            ...
            if (action === 'copy-link') handleCopyLink();
            else if (action === 'edit') handleOpenEditBookmarkModal(currentBookmarkId);
            else if (action === 'delete') handleDeleteBookmark();
        });
    }
    closeAllMainBookmarkMenus();
    ...
    bookmarks.forEach(bookmark => {
        ...
        img.src = '../assets/icons/404-icon16.webp';
        setFaviconWithFallback(img, bookmark.url);
    ...
    if (bookmarks.length > 0) {
        ...
        addBtn.setAttribute('data-tooltip', i18nData['bookmark.add']?.[lang] || 'Tambah Bookmark');

        const moreBtn = document.createElement('button');
        moreBtn.className = 'bookmark-more-btn';
        moreBtn.innerHTML = '&#8942;';
        moreBtn.onclick = () => openModal(bookmarkListModal.overlay);
        moreBtn.setAttribute('data-tooltip', i18nData['bookmark.open']?.[lang] || 'Lihat Bookmark');

        elements.mainPageBookmarkControls.appendChild(addBtn);
        elements.mainPageBookmarkControls.appendChild(moreBtn);
    } else {
        const addBtn = document.createElement('button');
        addBtn.className = 'bookmark-add-btn';
        addBtn.innerHTML = '<span>+</span>';
...
export function handleOpenAddBookmarkModal() {
...
export async function handleSaveBookmark() {
    ...
    const urlExists = bookmarks.some(b => 
        b.url.trim().toLowerCase() === normalizedUrl && 
        (isEditing ? Number(b.id) !== Number(currentBookmarkId) : true)
    );
    ...
    if (isEditing) {
        const idToEdit = Number(currentBookmarkId);
        const originalBookmark = bookmarks.find(b => Number(b.id) === idToEdit);
        if (originalBookmark) {
            const originalDomain = getDomainFromUrl(originalBookmark.url);
            const newDomain = getDomainFromUrl(url);

            if (originalDomain && newDomain && originalDomain !== newDomain) {
                await deleteFaviconFromCache(originalDomain);
            }
        }
    }
    ...
    if (isEditing) {
        const idToEdit = Number(currentBookmarkId);
        const index = tempBookmarks.findIndex(b => Number(b.id) === idToEdit);
        if (index > -1) {
            tempBookmarks[index] = { ...tempBookmarks[index], id: idToEdit, name, url };
        }
    } else {
    ...
    currentBookmarkId = null;
    bookmarkModal.nameInput.blur();
    bookmarkModal.urlInput.blur();
}
...
export async function confirmDeleteBookmark() {
    ...
    if (confirmationModalPurpose === 'deleteSelectedBookmarks') {
        selectedBookmarkIds.forEach(id => idToDeleteSet.add(Number(id)));
    } else {
        idToDeleteSet.add(Number(currentBookmarkId));
    }
    
    const bookmarksToDelete = bookmarks.filter(b => idToDeleteSet.has(Number(b.id)));
    const domainsToDelete = bookmarksToDelete.map(b => getDomainFromUrl(b.url)).filter(Boolean);
    
    if (domainsToDelete.length > 0) {
        const deletionPromises = domainsToDelete.map(domain => deleteFaviconFromCache(domain));
        await Promise.all(deletionPromises);
    }
    
    const tempBookmarks = bookmarks.filter(b => !idToDeleteSet.has(Number(b.id)));
...
async function handleCopyLink() {
    ...
    if (bookmark) {
        try {
            ...
            log('error', 'log.error.copyLinkFailed', {}, err);
        }
    }
}
...
export function initializeBookmarks() {
    ...
    bookmarkModal.closeBtn.addEventListener('click', () => {
        closeModal(bookmarkModal.overlay);
        currentBookmarkId = null;
    });
    ...
    if (bookmarkListModal.addBtn) {
        bookmarkListModal.addBtn.addEventListener('click', handleOpenAddBookmarkModal);
    }
