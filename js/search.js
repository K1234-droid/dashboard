...
import { showToast, isValidURL, debounce, log } from './utils.js';
...
export async function initializeData() {
    try {
        const [characterPrompts, settingsData] = await Promise.all([
            ...
            loadSettings(['advancedPrompts', 'enableHistorySearch', 'bookmarks', 'enableBookmarkSearch',
                'enablePromptSearch', 'showBookmarks'])
        ]);

        const builderPrompts = settingsData.advancedPrompts || [];
        const bookmarks = settingsData.bookmarks || [];
        const isBookmarkSearchEnabled = settingsData.enableBookmarkSearch !== false && settingsData.showBookmarks !== false;
        ...
        log('error', 'log.error.failedToInitSearch', {}, error);
...
function displayInitialResults(localResults) {
...
function updateSelection(updateInput = false) {
    ...
    if (selectedIndex >= 0 && selectedIndex < items.length) {
        ...
        if (updateInput) {
            if (selectedIndex >= 0 && selectedIndex < items.length) {
                const selectedData = currentResults[selectedIndex];
                if (selectedData) {
                    let displayText = '';
                    switch (selectedData.resultType) {
                        case 'bookmark':
                            displayText = selectedData.originalName || selectedData.text;
                            break;
                        case 'history':
                        case 'url':
                            displayText = selectedData.url;
                            break;
                        case 'web':
                            displayText = selectedData.query;
                            break;
                        case 'local':
                            displayText = selectedData.text;
                            break;
                        default:
                            displayText = selectedData.text || '';
                    }
                    footerSearch.input.value = displayText;
                }
            } else {
                footerSearch.input.value = originalQuery;
            }
        }
    }
}
...
export function initializeSearch() {
    ...
    footerSearch.input.addEventListener('input', updatePlaceholderVisibility);

    const debouncedHistorySearch = debounce(async (query) => {
        const historyResults = await performHistorySearch(query);
        
        if (query === footerSearch.input.value.trim() && historyResults.length > 0) {
            currentResults.push(...historyResults);
            appendResults(historyResults);
        }
    }, 300);
    ...
    footerSearch.input.addEventListener('input', async (event) => {
        ...
        displayInitialResults(localResults);
        ...
        if (itemId.startsWith('url-') || itemId.startsWith('hist-') || itemId.startsWith('web-')) {
            ...
            if (url.startsWith('file:///')) {
                try {
                    ...
                    showToast('popup.copy.localPathSuccess');
                } catch (err) {
                    log('error', 'log.error.copyLocalPathFailed', {}, err);
    ...
    footerSearch.resultsList.addEventListener('mouseleave', () => {
        selectedIndex = -1;
    ...
    document.addEventListener('keydown', (event) => {
        ...
        if (event.key === 'Escape') {
            ...
            if (resultsVisible) {
                footerSearch.resultsContainer.classList.remove('show');
                 if (footerSearch.input.value !== originalQuery) {
                    footerSearch.input.value = originalQuery;
                }
            } else if (hasText) {
